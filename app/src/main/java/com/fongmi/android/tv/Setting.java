...
public class Setting {
    ...
    // ========== 数据源过期锁定功能 - 丞相添加 ==========
    private static final String KEY_SOURCE_TIMESTAMP = "source_timestamp";
    private static final String KEY_SOURCE_EXPIRED = "source_expired";
     ...
     * 记录数据源配置时间戳
     */
    public static void setSourceTimestamp(long timestamp) {
        Prefers.put(KEY_SOURCE_TIMESTAMP, timestamp);
    ...
    public static long getSourceTimestamp() {
        return Prefers.getLong(KEY_SOURCE_TIMESTAMP, 0);
    }
     ...
     * @return true 表示已过期
     */
    public static boolean isSourceExpired() {
        long configTime = getSourceTimestamp();
        if (configTime == 0) return false; // 未配置不过期
        
        long currentTime = System.currentTimeMillis();
        long expireTime = configTime + (EXPIRE_DAYS * MILLIS_PER_DAY);
        return currentTime > expireTime;
    }

    /**
     * 获取剩余天数
     * @return 剩余天数，如果未配置返回 -1，如果已过期返回 0
     */
    public static int getRemainingDays() {
        long configTime = getSourceTimestamp();
        if (configTime == 0) return -1; // 未配置
        ...
        return (int) (remaining / MILLIS_PER_DAY) + 1; // 向上取整
    }

    /**
     * 设置过期状态
     */
    public static void setSourceExpired(boolean expired) {
        Prefers.put(KEY_SOURCE_EXPIRED, expired);
    }

    /**
     * 获取过期状态
     */
    public static boolean isSourceExpiredFlag() {
        return Prefers.getBoolean(KEY_SOURCE_EXPIRED, false);
    }
}
